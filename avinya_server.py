...
class Handler(BaseHTTPRequestHandler):
    ...
    def _handle_jsonbin(self, body):
        ...
        # JSONBin public bins — no auth key needed for read, master key for write
        # Using public bins keyed by SHA-256 hash — effectively private by obscurity
        target = f'https://api.jsonbin.io/v3/b/{jb_bin}'
        if jb_method == 'GET':
            target += '/latest'
        headers = {'Content-Type':'application/json', 'X-Bin-Private':'false'}
        data = None
        if jb_body:
            data = json.dumps(jb_body).encode()
        req = ur.Request(target, data=data, headers=headers, method=jb_method)
        try:
